...
const AllStudentsReport: React.FC<AllStudentsReportProps> = ({ students }) => {
  ...
  const getGoalsAbove75Percent = (student: Student) => {
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ textAlign: 'center', mb: 4 }}>
        <Typography variant="h3" component="h1" sx={{ '@media print': { fontSize: '2rem' } }}>
          IEP Progress Reports
        </Typography>
        <Typography variant="h6" color="text.secondary" sx={{ '@media print': { fontSize: '1rem' } }}>
          Generated on {new Date().toLocaleDateString()}
        </Typography>
        <Typography variant="subtitle1" color="text.secondary" sx={{ '@media print': { fontSize: '0.9rem' } }}>
      ...
      {students.map((student, studentIndex) => {
        ...
        return (
          <Box 
            ...
            sx={{ 
              '@media print': { 
                pageBreakBefore: studentIndex > 0 ? 'always' : 'auto',
          ...
          >
            {/* Student Header */}
            <Box sx={{ mb: 3 }}>
              <Typography variant="h4" component="h2" sx={{ '@media print': { fontSize: '1.5rem' } }}>
                Progress Report
              </Typography>
              <Typography variant="h5" color="text.secondary" sx={{ '@media print': { fontSize: '1.2rem' } }}>
                {student.studentName}
              </Typography>
            </Box>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap', gap: 2 }}>
                ...
                <Box sx={{ textAlign: 'center', minWidth: 120 }}>
                  <Typography variant="h3" color="info.main" sx={{ '@media print': { fontSize: '2rem', color: '#000 !important' } }}>
                    {getGoalsAbove75Percent(student)}%
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ '@media print': { color: '#000 !important' } }}>
                        ...
                        <Box sx={{ mb: 2, '@media print': { mb: 1.5 } }}>
                          ...
                          <Box sx={{ 
                            ...
                            {summary.goal.notes
                              ...
                              .slice(0, 3) // Show only the 3 most recent notes in all students report
                              .map((note, index) => (
                                <Box key={note.noteId} sx={{ 
                                  mb: index < Math.min(2, summary.goal.notes!.length - 1) ? 1 : 0,
                                  '@media print': { mb: index < Math.min(2, summary.goal.notes!.length - 1) ? 0.5 : 0 }
                                }}>
                            ...
                            {summary.goal.notes.length > 3 && (
                              <Typography variant="caption" color="text.secondary" sx={{
                                ...
                                ... and {summary.goal.notes.length - 3} more notes
                              </Typography>
                            )}
                          </Box>
                        </Box>
